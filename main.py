...
# Use the pinned version of llama-index (v0.5.6)
...
# Provided OpenAI API key (hardcoded as requested)
OPENAI_API_KEY = (
    "sk-proj-"
...
def build_index(products_df):
    ...
    Each product row becomes a Document containing its details.
    """
    documents = []
...
def enhance_product_display(response_text, products_df):
    """
    Replace product title markers (wrapped in <h3> tags) with a styled product card.
    ...
    for match in reversed(matches):
        ...
        if not product_match.empty:
            ...
            """
            # Replace the matched <h3> tag with the complete product card HTML.
...
def init_session():
    ...
    Initialize Streamlit session state:
      ...
      - Build the LlamaIndex vector store index
    """
...
def main():
    ...
    if prompt := st.chat_input("Ask about products..."):
        ...
        # Query the index (return top 3 matching products)
        response = st.session_state.index.query(prompt, similarity_top_k=3)
        matched_docs = response.source_nodes
        
        # Construct a response by wrapping product titles in <h3> tags
        if matched_docs:
            new_response = ""
